...
โปรเจกต์นี้เป็นระบบ ETL Pipeline เต็มรูปแบบที่ออกแบบเพื่อประมวลผลชุดข้อมูล Streaming Interactions Sample จาก Kaggle โดยประกอบด้วยสามขั้นตอนหลัก ได้แก่ Ingest (โหลดข้อมูล CSV เข้าฐานข้อมูล), Transform (ทำความสะอาดข้อมูลและ Feature Engineering) และ Publish (ส่งออกข้อมูลไปยัง Google Sheets สำหรับงาน BI/Visualization)
...
* เปิด Google Sheets
* ตั้งชื่อให้ตรงกับ `.env` เช่น `Streaming`
...
```
Streaming-pipeline@myproject.iam.gserviceaccount.com
```
...
```
docker exec -it Streaming_pipeline bash
...
* อ่านข้อมูลจาก CSV (`data/100k_a.csv.csv`)
* โหลดเข้า PostgreSQL table:
  `raw_data.raw_data_100k`
...
ระบบนี้เป็นตัวอย่างของ Data Engineering Pipeline ที่มีคุณภาพ production-ready โครงสร้างชัดเจน รองรับการขยายในอนาคต และเข้าใจง่ายสำหรับผู้เริ่มต้น
